...
function QwRunner( options ) {
    ...
    this.stopTimeout = options.workerStopTimeout || 50;         // ms before runner tells the worker to exit (TODO: maybe not?)
...
QwRunner.prototype.endWorkerProcess = function endWorkerProcess( worker, optionalCallback ) {
    ...
    if (++worker._useCount < this.maxUseCount) {
        ...
        var stopTimer = setTimeout(this.sendTo, this.stopTimeout, worker, { pid: worker.pid, qwType: 'exit' }).unref();
